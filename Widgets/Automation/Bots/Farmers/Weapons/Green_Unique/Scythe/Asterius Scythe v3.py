"""Asterius Scythe Farm v3 -- single-file version with all logic inlined.
...
class ScenarioLearner:
    ...
    _MUTATE_AFTER_RUNS = 999999   # mutation disabled -- stabilize first
    _MUTATE_EVERY_N_RUNS = 999999 # mutation disabled
    _MAX_STRATEGIES = 3           # only base strategies
    _PARAM_RANGES = {             # valid parameter bounds
        "enemy_threshold": (3, 15),
        "hp_threshold": (0.30, 0.80),
    }

    def maybe_mutate(self):
        """After enough data, create a mutated variant of the best strategy.
        Called after each run. Spawns at most one new strategy per N runs."""
        total = self.total_runs
        if total < self._MUTATE_AFTER_RUNS:
            return
        if total % self._MUTATE_EVERY_N_RUNS != 0:
            return
        if len(self.strategies) >= self._MAX_STRATEGIES:
            # Remove the worst strategy (lowest success rate with enough trials)
            worst_name, worst_rate = None, 2.0
            for name, s in self.strategies.items():
                if s.trials >= self.min_trials and s.success_rate < worst_rate:
                    worst_rate = s.success_rate
                    worst_name = name
            if worst_name:
                del self.strategies[worst_name]
                self._order.remove(worst_name)
                ConsoleLog("ScenarioLearner",
                           f"Removed worst strategy '{worst_name}' ({worst_rate:.0%})",
        ...
        # Mutate: nudge each numeric param by +/- 10-20%
        new_params = dict(best["params"])
        new_params["hard_mode"] = True  # always HM
        for key, (lo, hi) in self._PARAM_RANGES.items():
            if key in new_params:
                val = new_params[key]
                delta = val * random.uniform(-0.20, 0.20)
                new_val = max(lo, min(hi, val + delta))
                # Round int params, keep float params to 2 decimals
                if isinstance(val, int) or (isinstance(val, float) and val == int(val) and key == "enemy_threshold"):
        ...
        gen = total // self._MUTATE_EVERY_N_RUNS
        new_name = f"mutant_g{gen}"
        if new_name in self.strategies:
            return  # already exists

        self.add_strategy(new_name, new_params)
        ConsoleLog("ScenarioLearner",
                   f"Spawned '{new_name}' from '{best['name']}': {new_params}",
                   Console.MessageType.Info)
        self.save()
    ...
    def _handle_zone_entry_mobs():
        ...
        while time.time() < _sprint_end:
            ...
            try:
                ...
                ConsoleLog(bot_name, f"Micro-step move error: {e}", Console.MessageType.Warning)
        ...
        _extended_end = time.time() + 30  # hard timeout to prevent infinite loop if bodyblocked
        while _dist_to_portal() < _PORTAL_SAFE_DIST and _dist_to_safe() > 500 and time.time() < _extended_end:
            if _wrong_map():
...
def create_patrol_intercept(bot, bot_name, waypoints, boss_state,
    ...
    def _intercept_patrol():
        ...
        Top-level try/except prevents the coroutine from silently dying —
        a dead coroutine causes HasManagedCoroutine to return False,
        which triggers _is_done() → premature resign on Run 2+."""
        try:
            yield from _intercept_patrol_inner()
        except Exception as e:
            ConsoleLog(bot_name,
                       f"CRITICAL: patrol coroutine crashed: {e}",
                       Console.MessageType.Error)
        # Keep alive regardless — prevents HasManagedCoroutine returning False
        while True:
            yield from Routines.Yield.wait(5000)

    def _intercept_patrol_inner():
        """Inner patrol logic — separated so _intercept_patrol can catch all exceptions."""
        ...
        try:
            ...
            ConsoleLog(bot_name, f"Closest waypoint calc error: {e}", Console.MessageType.Warning)
            start_idx = 0
        ...
        for loop_count in range(max_loops):
            ...
            for _i, (wx, wy) in enumerate(wp_list):
                ...
                # Track current waypoint for debug
                boss_state["waypoint"] = _i
                boss_state["waypoint_total"] = total_wp

                # Progress log every 10 waypoints
                if _i > 0 and _i % 10 == 0:
                    ConsoleLog(bot_name, f"Patrol: waypoint {_i}/{total_wp}",
                               Console.MessageType.Info)

                # Move to next waypoint with random jitter (+-50 units)
        ...
        ConsoleLog(bot_name, "Patrol loops exhausted -- boss not found",
                   Console.MessageType.Warning)
        # Return to outer _intercept_patrol which keeps the coroutine alive
...
# ===========================================================================
LOOT_SWEEP_RADIUS = 300      # units — covers full auto-loot pickup range
LOOT_LEG_TIMEOUT_S = 8       # max seconds per sweep leg before skipping
LOOT_OOC_TIMEOUT_S = 20      # max seconds waiting for out-of-combat
LOOT_FINAL_WINDOW_S = 5      # seconds after OOC for auto-loot to finish pickups
LOOT_LEG_PAUSE_MS = 1200     # pause between sweep legs — auto-loot needs time per item
...
def create_loot_and_resign(bot, bot_name, sweep_radius=LOOT_SWEEP_RADIUS,
    ...
    def _loot_sweep(cx, cy):
        ...
        if _is_player_dead():
            ConsoleLog(bot_name, "Player dead -- skipping loot sweep",
                       Console.MessageType.Warning)
            return
        angles = [0, 90, 180, 270]
        total = len(angles)
        for i, angle_deg in enumerate(angles):
            ...
            tx = cx + sweep_radius * math.cos(rad)
            ty = cy + sweep_radius * math.sin(rad)
            # Raw Player.Move — no A*, no FSM, no trapezoid lookup.
            # 300u is too short for pathfinding and A* crashes the FSM.
            try:
                Player.Move(tx, ty)
            except Exception:
                continue
            move_end = time.time() + leg_timeout_s
            while time.time() < move_end:
                if not Map.IsMapReady() or Map.IsMapLoading() or not Map.IsExplorable():
                    return
                if _is_player_dead():
                    return
                try:
                    px, py = Player.GetXY()
                    if Utils.Distance((px, py), (tx, ty)) < 100:
                        break
                except Exception:
                    break
                yield from Routines.Yield.wait(400)
            # Pause for auto-loot pickup
            yield from Routines.Yield.wait(LOOT_LEG_PAUSE_MS)
    ...
    # Loot-on-ground gate: block resign while items are within Earshot
    _LOOT_CHECK_INTERVAL_MS = 2000   # check every 2 seconds
    _LOOT_MAX_WAIT_S = 60            # hard cap to prevent infinite wait
    _LOOT_EARSHOT = Range.Earshot.value

    def _wait_for_loot_pickup():
        """Coroutine: block until no pickable items remain on the ground.
        Only counts items within Earshot of ALIVE players — items near
        dead players are ignored since they can't pick them up.
        Checks every 2s. Hard timeout after 60s."""
        loot_start = time.time()
        while (time.time() - loot_start) < _LOOT_MAX_WAIT_S:
            if not Map.IsMapReady() or Map.IsMapLoading() or not Map.IsExplorable():
                return
            try:
                item_agents = AgentArray.GetItemArray()
                if not item_agents:
                    ConsoleLog(bot_name, "No items on ground -- ready to resign",
                               Console.MessageType.Info)
                    return

                # Collect positions of alive players only
                alive_positions = []
                try:
                    for player in Party.GetPlayers():
                        try:
                            aid = Party.Players.GetAgentIDByLoginNumber(player.login_number)
                            if aid > 0 and Agent.IsAlive(aid):
                                alive_positions.append(Agent.GetXY(aid))
                        except Exception:
                            continue
                except Exception:
                    pass
                ...
                if not alive_positions:
                    ConsoleLog(bot_name, "All players dead -- no one can loot, proceeding",
                               Console.MessageType.Warning)
                    return

                # Count items reachable by at least one alive player
                pickable = 0
                for item_id in item_agents:
                    try:
                        ixy = Agent.GetXY(item_id)
                        for pxy in alive_positions:
                            if Utils.Distance(pxy, ixy) <= _LOOT_EARSHOT:
                                pickable += 1
                                break
                    except Exception:
                        continue
                ...
                if pickable == 0:
                    ConsoleLog(bot_name, "No pickable items near alive players -- ready to resign",
                               Console.MessageType.Info)
                    return

                elapsed = int(time.time() - loot_start)
                if elapsed > 0 and elapsed % 10 == 0:
                    ConsoleLog(bot_name,
                               f"Loot wait: {pickable} items near {len(alive_positions)} alive players ({elapsed}s)",
                               Console.MessageType.Info)
            except Exception as e:
                ConsoleLog(bot_name, f"Loot check error: {e}", Console.MessageType.Warning)
                return
            yield from Routines.Yield.wait(_LOOT_CHECK_INTERVAL_MS)

        ConsoleLog(bot_name, f"Loot wait timeout ({_LOOT_MAX_WAIT_S}s) -- proceeding to resign",
                   Console.MessageType.Warning)

    def _loot_and_resign():
        """Top-level coroutine: sweep, OOC wait, loot gate, done."""
        state["done"] = False
        try:
            if not Map.IsMapReady() or not Map.IsExplorable():
                return
            ...
            # Gate: don't resign while items are still on the ground near any player
            yield from _wait_for_loot_pickup()
        except Exception as e:
...
def is_intercept_or_death_done():
    ...
    if result:
        has_coro = bot.config.FSM.HasManagedCoroutine("InterceptPatrol")
        ConsoleLog(BOT_NAME,
...
reset_farm = create_farm_reset(
    bot, boss_state,
    coroutine_names=["InterceptPatrol", "AsteriusScanner", "DeathMonitor", "OutpostResumeGuard"],
...
def farm_scythes(bot_ref: Botting) -> None:
    ...
    bot_ref.States.AddManagedCoroutine('AsteriusScanner', bg_scanner_fn)
    bot_ref.States.AddManagedCoroutine('DeathMonitor', death_monitor_state)
    ...
    def _outpost_fsm_resume_guard():
        while True:
            ...
            yield from Routines.Yield.wait(1000)
    bot_ref.States.AddManagedCoroutine('OutpostResumeGuard',
    ...
    # Post-kill loot phase: sweep cardinal directions around kill point,
    # wait for out-of-combat, then proceed to resign.
    # Replaces the old 5s fixed wait which left loot on the ground.
    bot_ref.States.AddCustomState(loot_custom_state, "Loot sweep + OOC wait")
    bot_ref.Wait.UntilCondition(is_loot_or_death_done, duration=1000, max_timeout_s=60)
    ...
    def _post_kill_resign_and_restart():
        ...
        # 4. Direct jump back to strategy selection — find the [H]Strategy_*
        #    step dynamically so this doesn't break when headers are added/removed.
        _target = None
        for _sn in bot.config.FSM.get_state_names():
            if _sn.startswith("[H]Strategy"):
                _target = _sn
                break
        if _target:
            bot.config.FSM.jump_to_state_by_name(_target)
        else:
            ConsoleLog(BOT_NAME, "CRITICAL: Strategy header not found in FSM!",
                       Console.MessageType.Error)
...
_debug_state_cache = {}         # cached state dict for throttled updates
_debug_state_last_update = 0.0  # timestamp of last _get_debug_state() call
_DEBUG_STATE_INTERVAL = 1.0     # min seconds between expensive state collections
# Pre-computed color tuples — avoid constructing Color objects every frame
_COL_WARN = Color(255, 100, 100, 255).to_tuple_normalized()
_COL_GOOD = Color(100, 255, 100, 255).to_tuple_normalized()
_COL_INFO = Color(180, 200, 255, 255).to_tuple_normalized()
_COL_GOLD = Color(255, 215, 0, 255).to_tuple_normalized()
...
def _get_debug_state() -> dict:
    ...
    # Slave status (per-account: alive/dead/map) — NO email prefixes (PII)
    slaves = []
    try:
            ...
            try:
                ...
                try:
                    if same_map and agent_id > 0:
                        dist = int(Utils.Distance(Player.GetXY(), Agent.GetXY(agent_id)))
                    else:
                        dist = -1
                except Exception:
                    dist = -1
                tag = "DEAD" if is_dead else ("OK" if same_map else "OOM")
                slaves.append({"idx": slave_idx, "tag": tag, "dist": dist})
            except Exception:
                continue
    except Exception:
        pass
...
def _draw_debug_window():
    """Draw a compact debug overlay with proper info hierarchy."""
    ...
    try:
        if not PyImGui.begin("Asterius##debug", True):
            PyImGui.end()
            return
    except Exception:
        return
    ...
    try:
        _debug_enabled = PyImGui.checkbox("Details", _debug_enabled)
        if not _debug_enabled:
            try:
                boss_quick = "KILLED" if boss_state.get("killed") else ("SPOTTED" if boss_state.get("spotted") else "Patrol")
                PyImGui.text_colored(boss_quick, _COL_GOOD if boss_state.get("killed") else _COL_DIM)
            except Exception:
                pass
            return

        # Throttle state collection
        global _debug_state_cache, _debug_state_last_update
        now = time.time()
        if now - _debug_state_last_update >= _DEBUG_STATE_INTERVAL:
            _debug_state_cache = _get_debug_state()
            _debug_state_last_update = now
        s = _debug_state_cache

        PyImGui.separator()

        # ═══ 1. BOSS STATUS (most important — is the run working?) ═══
        if s["boss_killed"]:
            PyImGui.text_colored("BOSS KILLED", _COL_GOOD)
        elif s["boss_spotted"]:
            PyImGui.text_colored(f"BOSS SPOTTED  dist {s['boss_dist']}", _COL_GOLD)
        else:
            PyImGui.text_colored(f"Scanning  wp {s['waypoint']}", _COL_DIM)

        # ═══ 2. STRATEGY + RUN (what are we doing?) ═══
        PyImGui.text(f"{s['strategy']}  HM  {s['run_time']}  Win {s['win_rate']}  #{s['total_runs']}")

        # ═══ 3. PARTY (is anyone dead?) ═══
        if s["death_triggered"]:
            PyImGui.text_colored("WIPE — resigning", _COL_WARN)
        else:
            dead_str = s['dead_players']
            dead_count = int(dead_str.split("/")[0]) if "/" in dead_str else 0
            if dead_count > 0:
                PyImGui.text_colored(f"Party {dead_str} dead", _COL_WARN)
            else:
                PyImGui.text_colored(f"Party {dead_str}", _COL_DIM)

        # Slave status — single line, no PII, color by worst status
        slaves = s.get("slaves", [])
        if slaves:
            parts = []
            has_dead = False
            for sl in slaves:
                ...
                dist = sl["dist"]
                if tag == "DEAD":
                    parts.append(f"S{sl['idx']}:DEAD")
                    has_dead = True
                elif tag == "OOM":
                    parts.append(f"S{sl['idx']}:OOM")
                else:
                    parts.append(f"S{sl['idx']}:{dist}u")
            col = _COL_WARN if has_dead else _COL_GOOD
            PyImGui.text_colored("  ".join(parts), col)

        PyImGui.separator()

        # ═══ 4. SESSION STATS ═══
        if _farm_tracker:
            PyImGui.text(f"Runs {s.get('session_runs', 0)}  Gold {s.get('session_gold', '0')}  G/h {s.get('session_gph', '0')}  Greens {s.get('session_greens', 0)}")

        # ═══ 5. INVENTORY ═══
        kits = "OK" if s['kits_done'] else "..."
        PyImGui.text(f"Bag {s['free_slots']}  Gold {s['gold']}  Kits {kits}")

        # ═══ 6. DIAGNOSTICS (low priority — only if space) ═══
        if s["in_combat"]:
            PyImGui.text_colored(f"Combat  {s['enemies']} enemies  HP {s['hp']}", _COL_WARN)

        if s["fsm_paused"]:
            PyImGui.text_colored("FSM PAUSED", _COL_WARN)

        # Last error — only show if recent (< 60s)
        if s["last_error"] and _last_error_time > 0 and (time.time() - _last_error_time) < 60:
            PyImGui.text_colored(f"ERR: {s['last_error'][:80]}", _COL_WARN)

        # Console dump (throttled)
        ...
        if now - _debug_last_console_dump >= _debug_console_interval:
            ...
            ConsoleLog(BOT_NAME,
                       ...
                       f"dead={s['dead_players']} run={s['run_time']}",
                       Console.MessageType.Info)
...
def tooltip():
    ...
    PyImGui.text_colored("Asterius Scythe Farmer v3", title_color.to_tuple_normalized())
    ImGui.pop_font()
    PyImGui.spacing()
    PyImGui.separator()
    PyImGui.text("multi-account bot to farm Asterius Scythe")
    PyImGui.spacing()
    PyImGui.bullet_text("v3: single-file, all logic inlined")
