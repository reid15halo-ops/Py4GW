...
from Py4GWCoreLib import Item, ItemArray, Inventory
...
def is_asterius_killed_or_time_elapsed():
    ...
    if elapsed > 180:
        if is_asterius_killed:
            _log(f"COUNTER WARN: runs_timeout about to bump but is_asterius_killed=True — watchdog double-reset?")
...
POST_KILL_DEAD_GRACE_S = 15.0  # how long to wait after leaving combat when party has dead members
POST_KILL_CLEAN_EXIT_BUFFER_S = 10.0
# 10s chosen: covers 95th percentile of observed phase values
# (73/299 runs ran phase>=30s, but buffer only needs to cover
# the steady-state slave pickup cadence)
...
def _loot_remaining_count() -> int:
    """Count lootable items within SafeCompass that ANY party member
    can pick up. LootConfig's GetfilteredLootArray has its party-aware
    branch commented out upstream, so master was blind to slave-
    assigned drops (owner_id == slave_agent_id). This counts them too.
    """
    try:
        loot_singleton = LootConfig()
        player_xy = Player.GetXY()
        dist_max = Range.SafeCompass.value

        # Collect valid party agent IDs (master + all heroes + all party players)
        party_ids = set()
        try:
            party_ids.add(Player.GetAgentID())
        except Exception:
            pass
        try:
            for h in GLOBAL_CACHE.Party.GetHeroes():
                party_ids.add(h.agent_id)
        except Exception:
            pass
        try:
            for p in GLOBAL_CACHE.Party.GetPlayers():
                aid = GLOBAL_CACHE.Party.Players.GetAgentIDByLoginNumber(p.login_number)
                if aid:
                    party_ids.add(aid)
        except Exception:
            pass

        # Walk raw item array, apply distance + blacklist + owner filter
        count = 0
        raw = AgentArray.GetItemArray()
        for item_id in raw:
            try:
                if not Agent.IsValid(item_id):
                    continue
                ix, iy = Agent.GetXY(item_id)
                dx, dy = player_xy[0] - ix, player_xy[1] - iy
                if dx * dx + dy * dy > dist_max * dist_max:
                    continue
                owner_id = Agent.GetItemAgentOwnerID(item_id)
                # Assigned to a party member OR unassigned (owner_id == 0)
                if owner_id != 0 and owner_id not in party_ids:
                    continue
                # Honor blacklist from LootConfig singleton
                item_data = Agent.GetItemAgentByID(item_id)
                if item_data is None:
                    continue
                model_id = Item.GetModelID(item_data.item_id)
                if loot_singleton.IsBlacklisted(model_id):
                    continue
                if loot_singleton.IsItemIDBlacklisted(item_id):
                    continue
                count += 1
            except Exception:
                continue
...
def reset_farm_flags():
    ...
    if is_asterius_killed:
        ...
        # Guard: reset_farm_flags ran without a kill. Expected after timeouts/wipes,
        # but if this fires frequently without a matching timeout/wipe bump,
        # something is double-resetting.
        _log(
            f"COUNTER: reset_farm_flags without kill "
            f"(timeouts={run_stats['runs_timeout']}, wipes={run_stats['runs_wiped']})"
        )
...
# Keeper whitelist: gold weapon MODELS we never auto-sell. Populate from
# farm drops you actually want. Everything else gets vendored.
# Leave empty to keep ALL golds (safe default until you review drops).
GOLD_KEEPER_MODELS: set[int] = set()
# Example once you decide: GOLD_KEEPER_MODELS = {2427, 24859}

# Flagged-for-sale item IDs collected during the diagnostic pass.
# Cleared on every outpost entry; consumed by the merchant-sell step.
_gold_sell_queue: list[int] = []


def _sell_flagged_golds():
    """Sell every item_id in _gold_sell_queue to the outpost merchant.
    Cleared after execution. No-op if queue is empty OR whitelist is
    empty (safety-first: empty whitelist means 'keep all')."""
    global _gold_sell_queue
    if not _gold_sell_queue:
        return
    if not GOLD_KEEPER_MODELS:
        _log("sell_flagged_golds: whitelist empty, skipping (safety)")
        _gold_sell_queue = []
        return
    try:
        # Use bot.Merchant helper if available, otherwise fall back
        sold = 0
        for item_id in list(_gold_sell_queue):
            try:
                bot.Merchant.MerchantSellItem(item_id, 1)
                sold += 1
            except Exception as e:
                _log(f"sell_flagged_golds: sell {item_id} failed: {e}")
        _log(f"sell_flagged_golds: sold {sold}/{len(_gold_sell_queue)} flagged golds")
    finally:
        _gold_sell_queue = []
...
def _dump_gold_mod_diagnostic():
    ...
    for item_id in items:
        try:
            ...
            if GOLD_KEEPER_MODELS and model_id not in GOLD_KEEPER_MODELS:
                _gold_sell_queue.append(item_id)
            golds_seen += 1
        except Exception as e:
...
def _verify_varajar_entry_or_log() -> None:
    ...
    if actual_map_id == VARAJAR_FELLS_MAP_ID:
        global _consecutive_olafstead_timeouts
        _consecutive_olafstead_timeouts = 0
...
def OnPartyWipe(bot: "Botting"):
    ...
    if is_asterius_killed:
        _log(f"COUNTER WARN: runs_wiped about to bump but is_asterius_killed=True — unexpected wipe after kill")
...
def _wait_for_party_regroup():
    ...
    while True:
        ...
        if elapsed_s >= REGROUP_MAX_WAIT_S:
            ...
            return
        try:
            if not Routines.Checks.Agents.InAggro(aggressive_only=True):
                # No immediate threat; regroup is nice-to-have, not required
                if elapsed_s > 2.0:  # let initial pixel-stack settle
                    _log(f"Party regroup: no aggro at elapsed={elapsed_s:.1f}s, continuing")
                    return
        except Exception:
            pass  # fall through to range check
        try:
...
# After this many consecutive Olafstead exit failures, use Map.Travel as
# fallback instead of retrying the same broken XYAndExitMap portal walk.
# Root cause (2026-04-10): crashed slave client blocks party from zoning.
MAX_CONSECUTIVE_OLAFSTEAD_TIMEOUTS = 3
_consecutive_olafstead_timeouts = 0
        ...
        try:
            ...
            if elapsed_in_step >= STEP_TIMEOUT_S:
                ...
                if current_map_id == OUTPOST_TO_TRAVEL:
                    global _consecutive_olafstead_timeouts
                    _consecutive_olafstead_timeouts += 1

                    if _consecutive_olafstead_timeouts >= MAX_CONSECUTIVE_OLAFSTEAD_TIMEOUTS:
                        # Repeated exit failures — likely a crashed slave blocking
                        # the zone. Try Map.Travel as fallback instead of walking
                        # to the portal again. See project_asterius_crashed_client_loop.md.
                        _log(
                            f"WATCHDOG TIMEOUT — step '{current_name}' stuck {elapsed_in_step:.0f}s in Olafstead "
                            f"({_consecutive_olafstead_timeouts}x consecutive). "
                            f"Using Map.Travel({VARAJAR_FELLS_MAP_ID}) as fallback."
                        )
                        try:
                            reset_farm_flags()
                        except Exception as e:
                        ...
                        try:
                            Map.Travel(VARAJAR_FELLS_MAP_ID)
                            _consecutive_olafstead_timeouts = 0
                        except Exception as e:
                            _log(f"watchdog: Map.Travel fallback failed: {e}")
                    else:
                        # Normal exit failure — reset and retry.
                        _log(
                            f"WATCHDOG TIMEOUT — step '{current_name}' stuck {elapsed_in_step:.0f}s in Olafstead "
                            f"(map_id={current_map_id}, attempt {_consecutive_olafstead_timeouts}/{MAX_CONSECUTIVE_OLAFSTEAD_TIMEOUTS}). "
                            f"Resetting flags and re-zoning."
                        )
                        try:
                            reset_farm_flags()
                        except Exception as e:
                    ...
                    try:
                        ...
                        # Safety: never jump OUT of the post-kill gate (Wait.UntilCondition loop).
                        # If that step times out, more likely the gate predicate wedged — we want
                        # to log and hard-cap, not silently skip loot.
                        if current_name and "UntilCondition" in current_name:
                            _log(
                                f"WATCHDOG: refusing to auto-advance past '{current_name}' "
                                f"(Wait.UntilCondition — would skip loot gate). Resigning via hard cap."
                            )
                            # Force is_ready_to_resign to fire next tick
                            POST_KILL_STATE["phase_start_ts"] = time.time() - POST_KILL_MAX_WAIT_S - 1
                            step_started_ts = time.time()
                            last_warn_step = None
                            continue
...
KILL_ROUTE_NEXT_STEP = "Log route done"
# Boss can physically never be dead in the first N seconds of the kill route —
# the party hasn't walked far enough to engage yet. Guards against stale
# is_asterius_killed leaking from a previous run. Empirically fastest real
# kill observed was ~30s into the route; 15s is safely below that floor.
KILL_PREEMPT_MIN_ELAPSED_S = 30.0


def _resolve_kill_route_step_name() -> str:
    """Walk the FSM and return the actual step name tagged 'Kill Route'.

    bot.Move.FollowAutoPath(TRAVEL_PATH, 'Kill Route') is auto-numbered
    'FollowAutoPath_N' where N depends on how many FollowAutoPath calls
    preceded it (including those injected by Templates.Routines.Prepare
    ForFarm). Resolving at runtime prevents a recurrence of the April
    2026 'FollowAutoPath_7 vs _8' bug.
    """
    try:
        fsm = bot.config.FSM
        # FSM iterates 1-based. We walk until None.
        idx = 1
        last_autopath = None
        while True:
            name = fsm.get_state_name_by_number(idx)
            if not name:
                break
            if name.startswith("FollowAutoPath_"):
                # Check the step's step_name attribute via the FSM internals
                # if exposed; otherwise match by position — the 'Kill Route'
                # FollowAutoPath is always the last one in the FSM.
                last_autopath = name
            idx += 1
        return last_autopath if last_autopath else "FollowAutoPath_8"
    except Exception as e:
        _log(f"kill-preempt: FSM probe failed ({e}) — falling back to _8")
        return "FollowAutoPath_8"


def kill_route_preempt_watchdog():
    """Preempt the kill-route step when Asterius is already confirmed dead.

    The kill-route FollowAutoPath step (bot.Move.FollowAutoPath kill route) is
    abused as a 180s timer — the main step_watchdog will eventually advance
    it, but in practice Asterius is usually dead 60–120s earlier. This
    coroutine polls at 2 Hz and jumps to 'Log route done' as soon as the
    existing is_asterius_killed global (set by handle_asterius_killed_en_route
    via Agent.IsDead) flips True, while the current step is still the kill
    route. Saves ~60–120s per run.

    Guards:
      - Only jumps FROM the kill-route FollowAutoPath step.
      - Latches per visit: won't double-jump; latch resets when the step
        name changes away from the kill-route FollowAutoPath step so the next
        run works.
      - Tracks its own independent step-start timer (no race with
        step_watchdog).
    """
    last_step_name: str | None = None
    step_started_ts: float = time.time()
    jumped_this_visit: bool = False
    nonlocal_target_placeholder = None  # reserved
    _resolved_kill_route_step_name: str = "FollowAutoPath_8"  # pessimistic default
    ...
    while True:
        try:
            ...
            # Re-resolve every 30s. The FSM step list can grow on first boot
            # (PrepareForFarm runs lazily) so the probe needs to tolerate a few
            # early None returns.
            if time.time() - _last_resolve_ts > 30.0:
                _resolved_kill_route_step_name = _resolve_kill_route_step_name()
            ...
            try:
                ...
                _log(f"kill-preempt: failed to read current step: {e}")
                continue

            if current_name != last_step_name:
                last_step_name = current_name
                step_started_ts = time.time()
                jumped_this_visit = False

            target = _resolved_kill_route_step_name  # captured once below
            if current_name != target:
                continue
            if jumped_this_visit:
                continue
            if not is_asterius_killed:
                continue

            # Verify the kill is REAL and CURRENT — not a stale flag from the
            # previous run. The global is_asterius_killed can be True from a
            # prior cycle if handle_asterius_killed_en_route re-detects a dead
            # agent before reset_farm_flags clears the flag. Two independent
            # guards:
            #   1. Elapsed floor: boss can't physically die in <30s of route
            #   2. Agent verification: asterius_agent_id must be valid AND the
            #      agent must actually be dead RIGHT NOW (not stale cache)
            elapsed_in_step = time.time() - step_started_ts
            if elapsed_in_step < KILL_PREEMPT_MIN_ELAPSED_S:
                continue
            if asterius_agent_id < 0:
                continue
            try:
                if not Agent.IsDead(asterius_agent_id):
                    continue
            except Exception:
                continue  # agent doesn't exist in this instance

            saved = max(0.0, STEP_TIMEOUT_S - elapsed_in_step)
            _log(
                f"Kill-detect preempt: Asterius confirmed dead, jumping "
                f"{_resolved_kill_route_step_name} → {KILL_ROUTE_NEXT_STEP} "
                f"(saved ~{saved:.0f}s)"
            )
            try:
                bot.States.JumpToStepName(KILL_ROUTE_NEXT_STEP)
                jumped_this_visit = True
            except Exception as e:
                _log(f"kill-preempt: JumpToStepName failed: {e}")
        except Exception as e:
            _log(f"kill-preempt: loop error: {e}")
            yield from Routines.Yield.wait(500)
...
def farm_scythes(bot: Botting) -> None:
    ...
    bot.States.AddManagedCoroutine('Kill route preempt (Asterius dead)', kill_route_preempt_watchdog)
    bot.States.AddCustomState(
        lambda: _log(f"FSM boot — kill-route step will be probed at runtime, not hardcoded"),
        "Log FSM boot"
    )
    ...
    bot.States.AddCustomState(_sell_flagged_golds, "Sell flagged gold weapons")
    bot.States.AddCustomState(lambda: _log("3s regroup pause"), "Log regroup")
