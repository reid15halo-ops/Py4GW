...
from Py4GWCoreLib.Builds.sf_vaettir_base import SFVaettirBase


class SF_Mes_vaettir_gda(SFVaettirBase):
    """Me/A Vaettir farmer with Great Dwarf Armor + Heart of Shadow for unstuck.

    Bar: Deadly Paradox | Shadow Form | Shroud of Distress | Mantra of Earth |
         Great Dwarf Armor | Arcane Echo | Wastrel's Demise | Heart of Shadow

    Attributes: 12 Shadow Arts, 9+1+1 Domination, 8+1 Fast Casting, 5 Inspiration
    """

    def __init__(self, match_only: bool = False):
        super().__init__(
            name="SF Mesmer Vaettir (GDA)",
            required_primary=Profession.Mesmer,
            required_secondary=Profession.Assassin,
            template_code="OQdDAQROqPP8Id2BwBiAvpLBDAA",
            required_skills=[
        ...
        self.great_dwarf_armor_slot = GLOBAL_CACHE.SkillBar.GetSlotBySkillID(GLOBAL_CACHE.Skill.GetID("Great_Dwarf_Armor"))
    ...
    def DefensiveActions(self):
        ...
        if (yield from Routines.Yield.Skills.IsSkillIDUsable(self.great_dwarf_armor)):
            yield from self._CastSkillID(self.great_dwarf_armor, log=False, aftercast_delay=500)

        # Heart of Shadow NEVER during kill phase — teleporting off the ball breaks the kill.
    ...
    def ProcessSkillCasting(self):
        ...
        # SF maintenance (top priority when in danger)
        ...
        # Shroud of Distress — recast on cooldown for damage reduction stacking
        ...
        # Mantra of Earth — maintain at all times
        ...
        # Great Dwarf Armor — maintain
        has_great_dwarf_armor = Routines.Checks.Effects.HasBuff(player_agent_id, self.great_dwarf_armor)
        if not has_great_dwarf_armor:
            if (yield from self._CastSkillID(self.great_dwarf_armor, log=False, aftercast_delay=500)):
                return

        # Heart of Shadow — unstuck during balling only, never during kill phase
        ...
        # Kill rotation: Arcane Echo + Wastrel's Demise (same as standard mesmer)
        if self.in_killing_routine and has_shadow_form and has_shroud_of_distress and has_mantra_of_earth:
            ...
            if target and shadow_form_buff_time_remaining >= 4000 and num_enemies >= 3:
                ...
                # Arcane Echo copies Wastrel's Demise when both are ready
                ...
                # Cast the echoed copy of Wastrel's Demise
