...
class FollowFormationPublisher:
    ...
    def _clear_follow_publish_state(self, all_accounts: AllAccounts, leader_account: AccountStruct, leader_index: int) -> None:
        """Reset the publisher's internal state and zero-out per-slot follow
        targets. MUST receive leader_index.
        ...
        portal-crossing bug where slow-loading slaves (Jini, Reid Paragorn,
        etc.) got skipped during map transitions because their MapID was
        still the old map while master's had already flipped to the new one.
        Their stale FollowPos (pointing at pre-zone coordinates near the
        portal on the OLD map) never got cleared, and their positive
        FollowMoveThreshold never got reset to the disabled sentinel. Once
        they finished loading the new map, their HeroAI.Follow() consumer
        read the stale target and walked them straight back through the
        portal. Clearing ALL active-party slots regardless of current-map
        state eliminates this race.
        """
    ...
    def _publish_impl(self) -> None:
        ...
        if (not Map.IsMapReady()) or Map.IsMapLoading() or (not Map.IsExplorable()):
            self._clear_follow_publish_state(all_accounts, leader_account, leader_index)
            return
