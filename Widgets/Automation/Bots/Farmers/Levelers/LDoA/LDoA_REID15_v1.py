...
import traceback
import random
from Py4GWCoreLib import Key, Map, ImGui, Botting, ActionQueue, Agent, Utils


# ========================== CONFIG ==========================
class Config:
    # Combat
    SKILL_INTERVAL = 1.5         # Base seconds between skill attempts
    ENEMY_RANGE_CLOSE = 1150     # Standard aggro range
    ENEMY_RANGE_MEDIUM = 1200    # Most farming routes
    ENEMY_RANGE_WIDE = 1500      # Red Iris Flowers (aggro only)
    ENEMY_RANGE_EXTRA = 2500     # Baked Husks (wide sweep)
    MAX_SKILL_SLOT = 8           # Default max skill slot
    HEALTH_FLEE_PCT = 0.45       # Flee threshold for Survivor title

    # Loot
    LOOT_RANGE_STANDARD = 1200   # Normal loot pickup range
    LOOT_RANGE_WIDE = 2500       # Flowers / Baked Husks
    LOOT_TYPES = {30, 10, 20, 9} # Consumable, Key, Dye, Salvage
    LOOT_PICKUP_DELAY = 1000     # Base ms between loot attempts

    # Movement
    STUCK_THRESHOLD = 20         # Units moved = considered stuck
    STUCK_TIMEOUT = 10.0         # Seconds before stuck reset
    COORD_JITTER = 25            # +/- game units added to waypoints

    # Items
    IMP_STONE_ID = 30847         # Fire Imp Summoning Stone model ID

    # Human-like behavior
    JITTER_LOW = 0.75            # Multiply timings by 0.75-1.35 for variance
    JITTER_HIGH = 1.35
    POST_COMBAT_PAUSE_CHANCE = 0.08  # 8% chance of 1-3s pause after clearing enemies
    SESSION_PAUSE_INTERVAL = 40      # Every ~40 runs, take a longer break
    SESSION_PAUSE_DURATION = (8, 20) # 8-20 second "afk" pause


# ========================== HUMANIZER ==========================
def jitter(value):
    """Add +/-30% variance to any numeric value (timing, delay, etc.)."""
    return value * random.uniform(Config.JITTER_LOW, Config.JITTER_HIGH)

def jitter_coord(x, y):
    """Add small random offset to coordinates for path variance."""
    j = Config.COORD_JITTER
    return (x + random.uniform(-j, j), y + random.uniform(-j, j))

_last_session_pause_run = 0

def maybe_post_combat_pause():
    """Occasionally pause after combat to simulate human 'looking around'."""
    if random.random() < Config.POST_COMBAT_PAUSE_CHANCE:
        return True  # Caller should add a short delay state
    return False

def should_session_pause(current_run):
    """Check if it's time for a longer 'afk' break."""
    global _last_session_pause_run
    if current_run > 0 and current_run - _last_session_pause_run >= Config.SESSION_PAUSE_INTERVAL:
        # Add some randomness so it's not every exactly N runs
        if random.random() < 0.3:  # 30% chance each run after threshold
            _last_session_pause_run = current_run
            return True
    return False

def get_session_pause_duration():
    """Get a random break duration in seconds."""
    lo, hi = Config.SESSION_PAUSE_DURATION
...
#UNNATURAL SEEDS
unnatural_seeds_coordinate_list = [(-7910, 1418),(-7555, -1752),(-8062, -3071),(-12424, -3125),(-10948, -5876),(-8324, -8323),(-13903, -2822),(-14847, -707),(-15180, 2976),(-16849, 7044),(-20397, 8233),(-20550, 6263),(-20964, 3164),(-18933, 3676),(-17504, 3399),(-20758, -1853),(-20250, -5231),(-19147, -6511),(-16715, -7669),(-16317, -9650),(-16739, -11548),(-12792, -14235),(-15378, -15096)]
...
#FUNCTIONS
def StartBot():
    global bot_vars, start_time, xp_at_start, xp_tracking_initialized
    bot_vars.bot_started = True
    start_time = time.time()
    xp_at_start = Player.GetExperience()
    xp_tracking_initialized = True
...
def quantityitem(model_id):
    item = Item.GetItemIdFromModelID(model_id)
    return Item.Properties.GetQuantity(item)

def equipitem(model_id, agent_id=None):
    item = Item.GetItemIdFromModelID(model_id)
    if agent_id is None:
        agent_id = Player.GetAgentID()
...
def FollowPathwithDelayTimer(path_handler,follow_handler, log_actions=False, delay=3000):
    ...
    if follow_delay_timer.IsStopped():
        ...
        return
    if follow_delay_timer.HasElapsed(int(jitter(delay))):
        follow_delay_timer.Stop()
        point = path_handler.advance()
        if point is not None:
            jx, jy = jitter_coord(point[0], point[1])
            follow_handler.move_to_waypoint(jx, jy)
            if log_actions:
...
def Survivor():
    """Returns True if health is below 45% (flee threshold for Survivor title)."""
    try:
        if not Map.IsMapReady() or Player.GetAgentID() <= 0:
            return False
        # Agent.GetHealth() returns 0.0-1.0 ratio directly
        health_ratio = Agent.GetHealth(Player.GetAgentID())
        return health_ratio < 0.45
    except Exception:
        return False
...
def IsSkillReady(slot):
    """Check if a skill slot has a skill and we have enough energy.
    Note: Skill.Data.GetRecharge() returns BASE recharge time, not current cooldown.
    We rely on the game client to reject skills still on cooldown."""
...
IsSkillReady2 = IsSkillReady

def _get_self_heal_slots(prof_id):
    """Return skill slots likely to be heals for this profession (Pre-Searing)."""
    if prof_id == Profession.Warrior.value:
        return [4]      # Healing Signet
    elif prof_id == Profession.Ranger.value:
        return [3]      # Troll Unguent
    elif prof_id == Profession.Monk.value:
        return [3, 4]   # Orison of Healing, Healing Breeze
    elif prof_id == Profession.Necromancer.value:
        return [1]      # Vampiric Gaze (life steal = sustain)
    elif prof_id == Profession.Mesmer.value:
        return [3]      # Ether Feast
    elif prof_id == Profession.Elementalist.value:
        return [3]      # Aura of Restoration
    return []

def use_best_skill(target_id, max_slot=8):
    """Smart skill usage: heals when low HP, then damage skills in order. Non-blocking."""
    my_id = Player.GetAgentID()

    # Don't use skills while already casting or knocked down
    if Agent.IsCasting(my_id) or Agent.IsKnockedDown(my_id):
        return False

    Player.ChangeTarget(target_id)

    # Self-heal priority when HP < 60%
    my_health = Agent.GetHealth(my_id)
    if my_health < 0.6:
        my_prof, _ = Agent.GetProfessionIDs(my_id)
        for slot in _get_self_heal_slots(my_prof):
            if slot <= max_slot and IsSkillReady(slot):
                SkillBar.UseSkill(slot, my_id)  # Target self for heals
                return True

    # Damage skills in priority order (slot 1 = highest)
...
# ========================== UNIFIED LOOT ==========================
def pick_up_loot(loot_range=Config.LOOT_RANGE_STANDARD):
    """Pick up nearest loot item within range. Filters by ownership and item type. Non-blocking."""
    my_id = Player.GetAgentID()
    my_xy = Agent.GetXY(my_id)

    item_array = AgentArray.GetItemArray()
    item_array = AgentArray.Filter.ByDistance(item_array, my_xy, loot_range)
    item_array = AgentArray.Filter.ByCondition(
        item_array, lambda aid: Agent.GetItemAgentOwnerID(aid) in (my_id, 0)
    )

    agent_to_item = {aid: Agent.GetItemAgentItemID(aid) for aid in item_array}
    valid_items = ItemArray.Filter.ByCondition(
        list(agent_to_item.values()),
        lambda iid: Item.GetItemType(iid)[0] in Config.LOOT_TYPES
    )
    valid_set = set(valid_items)
    lootable = [aid for aid, iid in agent_to_item.items() if iid in valid_set]
    lootable = AgentArray.Sort.ByDistance(lootable, my_xy)

    if lootable:
        target = lootable[0]
        if Player.GetTargetID() != target:
            Player.ChangeTarget(target)
            loot_timer.Reset()
            return
        if loot_timer.HasElapsed(int(jitter(Config.LOOT_PICKUP_DELAY))) and Player.GetTargetID() == target:
            Keystroke.PressAndRelease(Key.Space.value)
            loot_timer.Reset()

# Backwards-compatible aliases
def handle_loot():
    pick_up_loot(Config.LOOT_RANGE_STANDARD)

def handle_lootFlower():
    pick_up_loot(Config.LOOT_RANGE_WIDE)
...
def pick_best_target(enemies):
    """Pick target: wounded first, then casters, then nearest."""
    if not enemies:
        return None
    # Finish wounded targets (< 50% HP)
    wounded = AgentArray.Filter.ByCondition(enemies, lambda eid: Agent.GetHealth(eid) < 0.5)
    if wounded:
        return AgentArray.Sort.ByHealth(wounded)[0]
    # Prioritize casters (more dangerous, lower HP)
    try:
        casters = AgentArray.Filter.ByCondition(enemies, lambda eid: Agent.IsCaster(eid))
        if casters:
            return AgentArray.Sort.ByDistance(casters, Player.GetXY())[0]
    except Exception:
        pass  # IsCaster may not exist in Pre-Searing -- fall through
    return enemies[0]

def combat_and_move(map_pathing, enemy_range=Config.ENEMY_RANGE_MEDIUM,
                    max_skill_slot=Config.MAX_SKILL_SLOT, loot=False,
                    loot_range=Config.LOOT_RANGE_STANDARD, aggro_only=False,
                    pet_guard=False, stuck_check=False):
    """Unified combat + loot + movement handler. Replaces all 9 specialized handlers.

    Args:
        map_pathing:    PathHandler for the current route
        enemy_range:    How far to scan for enemies
        max_skill_slot: Maximum skill bar slot to use (1-8)
        loot:           Whether to loot between fights
        loot_range:     Loot pickup radius
        aggro_only:     Only fight aggressive enemies (for flower routes)
        pet_guard:      Set pet to Guard mode when idle (Rangers)
        stuck_check:    Enable stuck detection (for gate opener / baked husks)
    """
    global FSM_vars
    my_id = Player.GetAgentID()
    my_xy = Agent.GetXY(my_id)
    my_p_prof, my_s_prof = Agent.GetProfessionIDs(my_id)
    is_ranger = my_p_prof == Profession.Ranger.value or my_s_prof == Profession.Ranger.value
    current_time = time.time()

    # Stuck detection (optional)
    if stuck_check:
        check_movement_stuck()

    # Find enemies
    enemy_array = AgentArray.GetEnemyArray()
    enemy_array = AgentArray.Filter.ByDistance(enemy_array, my_xy, enemy_range)
    enemy_array = AgentArray.Filter.ByAttribute(enemy_array, 'IsAlive')
    if aggro_only:
        enemy_array = AgentArray.Filter.ByAttribute(enemy_array, 'IsAgressive')
    enemy_array = AgentArray.Sort.ByDistance(enemy_array, my_xy)

    # No enemies -- post-combat pause, loot, move, manage pet
    if not enemy_array:
        # Post-combat micro-pause (human-like "looking around")
        if FSM_vars.current_target_id is not None and maybe_post_combat_pause():
            FSM_vars.last_skill_time = current_time + random.uniform(0.5, 2.0)

        FSM_vars.current_target_id = None
        FSM_vars.last_target_id = None
        FSM_vars.has_interacted = False
        if is_ranger and pet_guard:
            Party.Pets.SetPetBehavior(1, my_id)
        if loot:
            pick_up_loot(loot_range)
        Routines.Movement.FollowPath(map_pathing, FSM_vars.movement_handler)
        return

    # Pick target (smart: wounded > casters > nearest)
    if FSM_vars.current_target_id is None or not Agent.IsAlive(FSM_vars.current_target_id):
        FSM_vars.current_target_id = pick_best_target(enemy_array)
        FSM_vars.has_interacted = False

    if FSM_vars.current_target_id:
        target_id = FSM_vars.current_target_id

        if FSM_vars.last_target_id != target_id:
            FSM_vars.has_interacted = False
        ...
        if Agent.IsAlive(target_id):
            # Jittered skill interval (human-like variable reaction time)
            if current_time - FSM_vars.last_skill_time >= jitter(Config.SKILL_INTERVAL):
                use_best_skill(target_id, max_slot=max_skill_slot)
                FSM_vars.last_skill_time = current_time
            return

# ========================== BACKWARDS-COMPATIBLE WRAPPERS ==========================
# Thin wrappers so existing FSM lambdas work without changes.
def handle_map_path(p):
    combat_and_move(p, enemy_range=Config.ENEMY_RANGE_CLOSE)

def handle_map_path_loot(p):
    combat_and_move(p, loot=True)

def handle_map_path_Red_Iris_Flower(p):
    combat_and_move(p, enemy_range=Config.ENEMY_RANGE_WIDE, aggro_only=True,
                    loot=True, loot_range=Config.LOOT_RANGE_WIDE)

def warrior_handle_map_path(p):
    combat_and_move(p)

def mesmer_handle_map_path(p):
    combat_and_move(p, max_skill_slot=3, pet_guard=True)

def early_handle_map_path(p):
    combat_and_move(p, enemy_range=Config.ENEMY_RANGE_CLOSE, max_skill_slot=4,
                    loot=True, pet_guard=True)

def hamnet_handle_map_path(p):
    combat_and_move(p, max_skill_slot=4, pet_guard=True)
...
# Old handle_loot_baked_husk replaced by unified pick_up_loot alias above (line ~667)

def sweep_area_for_loot():
    """Non-blocking loot sweep."""
    pick_up_loot(Config.LOOT_RANGE_WIDE)
...
def check_movement_stuck():
    """Check if the bot is stuck and reset movement if needed."""
    global FSM_vars
    try:
        my_id = Player.GetAgentID()
        current_pos = Agent.GetXY(my_id)
        dist_moved = Utils.Distance(FSM_vars.last_position, current_pos)

        if dist_moved < Config.STUCK_THRESHOLD:
            if FSM_vars.position_stuck_time == 0.0:
                FSM_vars.position_stuck_time = time.time()
            elif time.time() - FSM_vars.position_stuck_time > Config.STUCK_TIMEOUT:
                Py4GW.Console.Log("Movement", "Bot stuck - resetting", Py4GW.Console.MessageType.Warning)
                FSM_vars.movement_handler.reset()
...
def handle_gadget_interaction():
    """Lever interaction: target with semicolon, activate with spacebar."""
    global FSM_vars
    current_time = time.time()
...
def handle_map_path_gate_opener(map_pathing):
    """Gate opener: just follow path with stuck detection, no combat."""
    global FSM_vars
    try:
        if not Map.IsMapReady() or Player.GetAgentID() <= 0:
            return
...
def handle_map_path_baked_husk(map_pathing):
    """Baked husk farming: wide enemy range, loot priority, stuck detection."""
    combat_and_move(map_pathing, enemy_range=Config.ENEMY_RANGE_EXTRA,
                    loot=True, loot_range=Config.LOOT_RANGE_WIDE, stuck_check=True)
...
FSM_vars.state_machine_warrior.AddState(name="CHECK NPC", execute_fn=lambda: handle_npc_interaction(), transition_delay_ms=1500, run_once=True)
...
FSM_vars.state_machine_ranger.AddState(name="CHECK NPC", execute_fn=lambda: handle_npc_interaction(), transition_delay_ms=1500, run_once=True)
...
FSM_vars.state_machine_monk.AddState(name="CHECK NPC", execute_fn=lambda: handle_npc_interaction(), transition_delay_ms=1500, run_once=True)
...
FSM_vars.state_machine_necromancer.AddState(name="CHECK NPC", execute_fn=lambda: handle_npc_interaction(), transition_delay_ms=1500, run_once=True)
...
FSM_vars.state_machine_mesmer.AddState(name="CHECK NPC", execute_fn=lambda: handle_npc_interaction(), transition_delay_ms=1500, run_once=True)
...
FSM_vars.state_machine_elementalist.AddState(name="CHECK NPC", execute_fn=lambda: handle_npc_interaction(), transition_delay_ms=1500, run_once=True)
...
FSM_vars.state_machine_lvl2_10.AddState(name="LOOTING DROPS", execute_fn=lambda: handle_loot(), transition_delay_ms=2000, run_once=True)
...
FSM_vars.state_machine_ResetQuest.AddState(name="CHECK NPC", execute_fn=lambda: handle_npc_interaction(), transition_delay_ms=1500, run_once=True)
...
FSM_vars.state_machine_lvl11_20.AddState(name="WAITING OUTPOST MAP", exit_condition=lambda: Map.IsMapReady() and LDoA_IsOutpost() and Party.IsPartyLoaded(), transition_delay_ms=3000, run_once=True)
...
FSM_vars.state_machine_lvl11_20.AddState(name="LOOTING DROPS", execute_fn=lambda: handle_loot(), transition_delay_ms=2000, run_once=True)
...
FSM_vars.state_machine_TamePet.AddState(name="CHECK QUEST", execute_fn=lambda: InteractPet(), transition_delay_ms=3000, run_once=True)
...
FSM_vars.state_machine_barradin.AddState(name="GOING BACK TO ASCALON", execute_fn=lambda: LDoA_TravelToOutpost(bot_vars.ascalon_map), exit_condition=lambda: Map.IsMapReady() and LDoA_IsOutpost() and Party.IsPartyLoaded(), transition_delay_ms=3000, run_once=True)
...
FSM_vars.state_machine_grandtour.AddState(name="WAITING OUTPOST MAP", exit_condition=lambda: Map.IsMapReady() and LDoA_IsOutpost() and Party.IsPartyLoaded(), transition_delay_ms=3000, run_once=True)
...
FSM_vars.state_machine_grandtour.AddState(name="WALKING TO WIZARD'S FOLLY", execute_fn=lambda: Routines.Movement.FollowPath(FSM_vars.foible_coordinate_list_two_pathing, FSM_vars.movement_handler), exit_condition=lambda: Routines.Movement.IsFollowPathFinished(FSM_vars.foible_coordinate_list_two_pathing, FSM_vars.movement_handler), run_once=False)
FSM_vars.state_machine_grandtour.AddState(name="WAITING OUTPOST MAP", exit_condition=lambda: Map.IsMapReady() and LDoA_IsOutpost() and Party.IsPartyLoaded(), transition_delay_ms=3000, run_once=True)
FSM_vars.state_machine_grandtour.AddState(name="GOING BACK TO ASHFORD ABBEY", execute_fn=lambda: LDoA_TravelToOutpost(bot_vars.abbey_map), exit_condition=lambda: Map.IsMapReady() and LDoA_IsOutpost() and Party.IsPartyLoaded(), transition_delay_ms=3000, run_once=True)
FSM_vars.state_machine_grandtour.AddState(name="WAITING OUTPOST MAP", exit_condition=lambda: Map.IsMapReady() and LDoA_IsOutpost() and Party.IsPartyLoaded(), transition_delay_ms=3000, run_once=True)
FSM_vars.state_machine_grandtour.AddState(name="GOING OUT ASHFORD ABBEY", execute_fn=lambda: Routines.Movement.FollowPath(FSM_vars.goingout_ashfordabbey, FSM_vars.movement_handler), exit_condition=lambda: Routines.Movement.IsFollowPathFinished(FSM_vars.goingout_ashfordabbey, FSM_vars.movement_handler) or (Map.IsMapReady() and Map.IsExplorable() and Party.IsPartyLoaded()), run_once=False)
FSM_vars.state_machine_grandtour.AddState(name="WAITING EXPLORABLE MAP", exit_condition=lambda: Map.IsMapReady() and Map.IsExplorable() and Party.IsPartyLoaded(), transition_delay_ms=3000, run_once=True)
FSM_vars.state_machine_grandtour.AddState(name="USING IMP STONE", execute_fn=lambda: useitem(30847), run_once=True)
...
FSM_vars.state_machine_grandtour.AddState(name="WAITING OUTPOST MAP", exit_condition=lambda: Map.IsMapReady() and LDoA_IsOutpost() and Party.IsPartyLoaded(), transition_delay_ms=3000, run_once=True)
FSM_vars.state_machine_grandtour.AddState(name="GOING BACK TO ASCALON", execute_fn=lambda: LDoA_TravelToOutpost(bot_vars.ascalon_map), exit_condition=lambda: Map.IsMapReady() and LDoA_IsOutpost() and Party.IsPartyLoaded(), transition_delay_ms=3000, run_once=True)
...
FSM_vars.state_machine_warrior_req.AddState(name="GOING BACK TO ASCALON", execute_fn=lambda: LDoA_TravelToOutpost(bot_vars.ascalon_map), exit_condition=lambda: Map.IsMapReady() and LDoA_IsOutpost() and Party.IsPartyLoaded(), transition_delay_ms=3000, run_once=True)
...
FSM_vars.state_machine_warrior_req.AddState(name="GOING TO BARRADIN", execute_fn=lambda: LDoA_TravelToOutpost(bot_vars.barradin_map), exit_condition=lambda: Map.IsMapReady() and LDoA_IsOutpost() and Party.IsPartyLoaded(), transition_delay_ms=3000, run_once=True)
...
FSM_vars.state_machine_warrior_noreq.AddState(name="GOING BACK TO ASCALON", execute_fn=lambda: LDoA_TravelToOutpost(bot_vars.ascalon_map), exit_condition=lambda: Map.IsMapReady() and LDoA_IsOutpost() and Party.IsPartyLoaded(), transition_delay_ms=3000, run_once=True)
...
FSM_vars.state_machine_ranger_req.AddState(name="GOING BACK TO ASCALON", execute_fn=lambda: LDoA_TravelToOutpost(bot_vars.ascalon_map), exit_condition=lambda: Map.IsMapReady() and LDoA_IsOutpost() and Party.IsPartyLoaded(), transition_delay_ms=3000, run_once=True)
FSM_vars.state_machine_ranger_req.AddState(name="GOING BACK TO ABBEY", execute_fn=lambda: LDoA_TravelToOutpost(bot_vars.abbey_map), exit_condition=lambda: Map.IsMapReady() and LDoA_IsOutpost() and Party.IsPartyLoaded(), transition_delay_ms=3000, run_once=True)
...
FSM_vars.state_machine_ranger_req.AddState(name="CHECK QUEST", execute_fn=lambda: InteractPet(), transition_delay_ms=3000, run_once=True)
...
FSM_vars.state_machine_ranger_noreq.AddState(name="GOING BACK TO ASCALON", execute_fn=lambda: LDoA_TravelToOutpost(bot_vars.ascalon_map), exit_condition=lambda: Map.IsMapReady() and LDoA_IsOutpost() and Party.IsPartyLoaded(), transition_delay_ms=3000, run_once=True)
FSM_vars.state_machine_ranger_noreq.AddState(name="GOING BACK TO ABBEY", execute_fn=lambda: LDoA_TravelToOutpost(bot_vars.abbey_map), exit_condition=lambda: Map.IsMapReady() and LDoA_IsOutpost() and Party.IsPartyLoaded(), transition_delay_ms=3000, run_once=True)
...
FSM_vars.state_machine_ranger_noreq.AddState(name="WALKING TO REGENT VALLEY", execute_fn=lambda: Routines.Movement.FollowPath(FSM_vars.tamepet_pathing_3, FSM_vars.movement_handler), exit_condition=lambda: Routines.Movement.IsFollowPathFinished(FSM_vars.tamepet_pathing_3, FSM_vars.movement_handler), run_once=False)
FSM_vars.state_machine_ranger_noreq.AddState(name="CHECK QUEST", execute_fn=lambda: InteractPet(), transition_delay_ms=3000, run_once=True)
...
FSM_vars.state_machine_monk_req.AddState(name="GOING BACK TO ASCALON", execute_fn=lambda: LDoA_TravelToOutpost(bot_vars.ascalon_map), exit_condition=lambda: Map.IsMapReady() and LDoA_IsOutpost() and Party.IsPartyLoaded(), transition_delay_ms=3000, run_once=True)
FSM_vars.state_machine_monk_req.AddState(name="GOING BACK TO ABBEY", execute_fn=lambda: LDoA_TravelToOutpost(bot_vars.abbey_map), exit_condition=lambda: Map.IsMapReady() and LDoA_IsOutpost() and Party.IsPartyLoaded(), transition_delay_ms=3000, run_once=True)
...
FSM_vars.state_machine_monk_noreq.AddState(name="GOING BACK TO ASCALON", execute_fn=lambda: LDoA_TravelToOutpost(bot_vars.ascalon_map), exit_condition=lambda: Map.IsMapReady() and LDoA_IsOutpost() and Party.IsPartyLoaded(), transition_delay_ms=3000, run_once=True)
FSM_vars.state_machine_monk_noreq.AddState(name="GOING BACK TO ABBEY", execute_fn=lambda: LDoA_TravelToOutpost(bot_vars.abbey_map), exit_condition=lambda: Map.IsMapReady() and LDoA_IsOutpost() and Party.IsPartyLoaded(), transition_delay_ms=3000, run_once=True)
...
FSM_vars.state_machine_necro_req.AddState(name="GOING BACK TO ASCALON", execute_fn=lambda: LDoA_TravelToOutpost(bot_vars.ascalon_map), exit_condition=lambda: Map.IsMapReady() and LDoA_IsOutpost() and Party.IsPartyLoaded(), transition_delay_ms=3000, run_once=True)
FSM_vars.state_machine_necro_req.AddState(name="GOING BACK TO ABBEY", execute_fn=lambda: LDoA_TravelToOutpost(bot_vars.abbey_map), exit_condition=lambda: Map.IsMapReady() and LDoA_IsOutpost() and Party.IsPartyLoaded(), transition_delay_ms=3000, run_once=True)
...
FSM_vars.state_machine_necro_noreq.AddState(name="GOING BACK TO ASCALON", execute_fn=lambda: LDoA_TravelToOutpost(bot_vars.ascalon_map), exit_condition=lambda: Map.IsMapReady() and LDoA_IsOutpost() and Party.IsPartyLoaded(), transition_delay_ms=3000, run_once=True)
FSM_vars.state_machine_necro_noreq.AddState(name="GOING BACK TO ABBEY", execute_fn=lambda: LDoA_TravelToOutpost(bot_vars.abbey_map), exit_condition=lambda: Map.IsMapReady() and LDoA_IsOutpost() and Party.IsPartyLoaded(), transition_delay_ms=3000, run_once=True)
...
class InventoryTracker:
    ...
    def get_farmed_items(self):
        ...
        try:
            current_counts = self.get_count_items()  # Single scan, not per-item
            for model_id in self.tracked_model_ids:
                initial_quantity = self.initial_quantities.get(model_id, 0)
                current_quantity = current_counts.get(model_id, 0)
...
def increment_run_counter():
    global run_counter
    run_counter += 1
    # Session pause: occasionally take a short break like a human would
    if should_session_pause(run_counter):
        pause_sec = get_session_pause_duration()
        Py4GW.Console.Log(module_name, f"Taking a {pause_sec:.0f}s break (run #{run_counter})", Py4GW.Console.MessageType.Info)
...
xp_at_start = 0
xp_tracking_initialized = False

def init_xp_tracking():
    """Initialize XP tracking at bot start."""
    global xp_at_start, xp_tracking_initialized
    if not xp_tracking_initialized:
        xp_at_start = Player.GetExperience()
        xp_tracking_initialized = True
...
def show_info_table_run():
    ...
    if not xp_tracking_initialized:
        init_xp_tracking()
    xp_gained = experience - xp_at_start
    xp_per_hour = int(xp_gained / (elapsed_time / 3600)) if elapsed_time > 10 else 0
    xp_to_next = max(0, required_experience - experience)

    # Time to next level estimate
    if xp_per_hour > 0:
        hours_to_next = xp_to_next / xp_per_hour
        ttl_formatted = time.strftime("%H:%M:%S", time.gmtime(hours_to_next * 3600))
    else:
        ttl_formatted = "--:--:--"

    # XP progress bar text
    xp_pct = min(100, int(experience / required_experience * 100)) if required_experience > 0 else 0

    data_info = [
        ("TIMER", elapsed_time_formatted),
        ("RUN COUNTER", str(run_counter)),
        ("AVG RUN TIME", average_run_time_formatted),
        ("LEVEL", f"{level}/20"),
        ("EXPERIENCE", f"{experience}/{required_experience} ({xp_pct}%)"),
        ("XP GAINED", f"+{xp_gained:,}"),
        ("XP / HOUR", f"{xp_per_hour:,}"),
        ("TIME TO NEXT LVL", ttl_formatted),
    ]
...
def main():
    ...
    try:
        ...
        if IsBotStarted():
            ...
            # DEATH RECOVERY: if dead during explorable map, return to outpost
            if Map.IsExplorable() and Death():
                Py4GW.Console.Log(module_name, "Death detected - returning to outpost", Py4GW.Console.MessageType.Warning)
                ResetEnvironment()
                # Don't stop the bot -- let the current FSM restart from the beginning
                return
